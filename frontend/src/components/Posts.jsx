...
const Posts = ({posts, showComments = false, thePostId = null, isProfilePage = false}) => {
  ...
  const bookMarkHandler = (postId) => {    
    ...
    if(index == -1){
      ...
      dispatch(updateUserData({userId : logedInUser._id, userData : bookMarks}))
    }else{
      const bookMarks = {
  ...
  const deleteHandler = (postId) => {
    ...
    const bookmarkIndex = logedInUser.bookmarkPosts.findIndex((id) => id == postId)
    if(likeIndex != -1){
      const likes = {
        likedPosts : logedInUser.likedPosts.filter((id) => id != postId)
      }
      ...
      <div className='row'>
        {posts.map((post) => (
          ...
          <div className='col-md-1'>
              <img src={`${post.userImage ? post.userImage : 'https://img.freepik.com/premium-photo/lego-figure-boy-wearing-glasses-jacket-with-hood_113255-94249.jpg?size=626&ext=jpg&ga=GA1.1.975262890.1723114701&semt=ais_hybrid'}`} alt='userImage'
              ...
              <div>
                <div>
                  {post.postMedia && <div>
                    ...
                    <img src={post.postMedia} alt='media' className='img-fluid py-3' /> 
                    :
                    <>
...
{/*                         value={editedPost ? editedPost : post.postText}  */}
                      ...
                      <div className="modal-footer">
                        ...
                        <button type="button" className="btn btn-primary" data-bs-dismiss="modal" onClick={() => handleUpdate(post._id)}>Save changes</button>
                      </div>
                    </div>
                  </div>
                </div>
          ...
          <div>
            ...
            {post.postComments.map(cmt => (
              <div className='row'>
                ...
                <div className='col-md-1'>
                  <img src={cmt.userImage} alt='userImage'
                  className='img-fluid rounded-circle'
                  />
                </div>
